# ============================================================
# Stefan_1D_2P_models_tf.py  — (Ngwenya supervised, fixed dT)
# PINN for 1D two-phase Stefan problem — laser melting Ti-6Al-4V
#
# Based on: Wang & Perdikaris JCP 2021
# Supervision: Ngwenya & Kahlen IMECE 2012 analytical reference
#
# Fixes vs. previous version:
#   - dT_l computed from actual Tl_surf_max (AI-dependent) so
#     net_Tl is never saturated for any intensity
#   - pde_scale for liquid uses actual liquid gradient scale
#   - curriculum: data supervision only for first warm_up iters,
#     then physics losses added gradually
# ============================================================
...
def ngwenya_X(t_arr, AI, ks, alpha_s, Tm, T0):
    """Melt depth X(t). AI = A*I_effective [W/m^2]."""
...
def ngwenya_Ts(z_arr, t_arr, AI, ks, alpha_s, Tm, T0):
    """Solid temp, semi-infinite flux-BC solution. Clamped to [T0, Tm]."""
...
def ngwenya_Tl(z_arr, X_arr, AI, kl, Tm):
    """Liquid temp, quasi-steady linear profile. Always >= Tm."""
    z_arr = np.asarray(z_arr, dtype=np.float64)
...
class Stefan1D2P(object):
    """
    PINN for 1-D two-phase Stefan (Wang & Perdikaris 2021)
    + Ngwenya & Kahlen analytical supervision.

    KEY FIX: dT_l is computed from the actual max liquid temperature
    (Tm + AI/kl * X_max), NOT from (Tm-T0). For high intensities
    the liquid can be orders of magnitude hotter than Tm.
    """
    ...
    def __init__(
        ...
        self.t_melt_f = t_melt_val   # Python float for print
    ...
    def X(self, t):
        ...
        # This matches the analytical solution: surface must first reach Tm
        # before any melting occurs.
    ...
    def train(self, data, iters=20000, lr=1e-3, print_every=1000, phys_weight=1.0):
        """
        phys_weight: float in [0, 1]
          0.0 = data supervision only (warm-up / curriculum phase 1)
          1.0 = full physics + data (phase 2)
        """
