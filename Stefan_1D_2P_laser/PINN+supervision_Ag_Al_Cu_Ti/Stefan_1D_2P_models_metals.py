# ============================================================
# Stefan_1D_2P_models_metals.py  — v4
...
def k_S(t_arr, AI_l, ks, alpha_s, Tm, T0, t_melt):
    """
    Melt front S(t) via quasi-steady approximation.
    AI_l = A_l * I  [W/m²]
    t_arr measured from absolute time (not from t_melt).
    S=0 for t <= t_melt.
    """
...
def k_Ts(z_arr, t_arr, AI_l, ks, alpha_s, Tm, T0, t_melt):
    """Solid temperature profile. Uses t' = t - t_melt."""
...
def k_Tl(z_arr, S_arr, AI_l, kl, Tm):
    """Liquid temperature, quasi-steady linear profile."""
    z_arr = np.asarray(z_arr, dtype=np.float64)
...
class StefanMetals:
    """
    PINN + analytical supervision for laser melting of metals.
    Mirrors Ti-6Al-4V approach (Stefan1D2P) but:
      - time domain [t_melt, t_max=10s]
      - supervision from k_S/Ts/Tl (same formula as Ngwenya)
      - separate rho_s, rho_l
    """
