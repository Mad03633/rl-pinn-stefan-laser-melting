...
def main():
    print("=" * 60)
    print("  PINN pure physics — Ti  [full training]")
    ...
    ax.plot(np.append(t_pre, t_melt_plot),
            ...
            color='#2ca02c', ls='-', lw=2.5, label='PINN — Ti')
    ...
    ax2.axhline(Tm, color='gray', ls=':', lw=1.0, label=f'Tm = 1940 K')
    ax2.set_xlabel("z (cm)", fontsize=12)
    ax2.set_ylabel("Temperature (K)", fontsize=12)
    ax2.set_title(f"T(z) at t = {t_max} s", fontsize=11)
    ax2.legend(fontsize=10); ax2.grid(True, alpha=0.3)

    plt.suptitle(f"Ti  |  I = {I_laser:.0e} W/m²  |  {elapsed/60:.1f} min",
                 fontsize=12, y=1.01)
    plt.tight_layout()
    plt.savefig("melt_depth_Ti_PINN_full.png", dpi=150, bbox_inches='tight')
    print("  Saved: melt_depth_Ti_PINN_full.png")
    plt.show()
