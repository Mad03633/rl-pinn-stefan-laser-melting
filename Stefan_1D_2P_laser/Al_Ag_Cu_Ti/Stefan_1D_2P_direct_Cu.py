# ============================================================
# Stefan_1D_2P_direct_Cu.py  — v5 full
# PINN pure physics — Cu
# I = 1e9 W/m²,  t in [t_melt=1.94, 10 s]
# ============================================================
...
def make_data(z_max, Nr=15000, N0=3000, Nbc=2000, NX=2000, seed=42):
    ...
    z_rl_max = 0.0701 * np.sqrt(tau_rl).clip(1e-9)
...
def main():
    print("=" * 60)
    print("  PINN pure physics — Cu  [full training]")
    ...
    print("\n--- Phase 2b: full phys   lr=5e-4  phys=1.0 ---")
    ...
    print(f"  Отклонение: {abs(S_pinn[-1]-S_anal[-1])/S_anal[-1]*100:.1f}%")
    ...
    ax.plot(np.append(t_pre, t_melt_plot),
            ...
            color='#d62728', ls='-.', lw=2.5, label='PINN — Cu')
    ...
    ax2.axhline(Tm, color='gray', ls=':', lw=1.0, label=f'Tm = 1358 K')
    ax2.set_xlabel("z (cm)", fontsize=12)
    ax2.set_ylabel("Temperature (K)", fontsize=12)
    ax2.set_title(f"T(z) at t = {t_max} s", fontsize=11)
    ax2.legend(fontsize=10); ax2.grid(True, alpha=0.3)

    plt.suptitle(f"Cu  |  I = {I_laser:.0e} W/m²  |  {elapsed/60:.1f} min",
                 fontsize=12, y=1.01)
    plt.tight_layout()
    plt.savefig("melt_depth_Cu_PINN_full.png", dpi=150, bbox_inches='tight')
    print("  Saved: melt_depth_Cu_PINN_full.png")
    plt.show()
